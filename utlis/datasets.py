...
def train_test_model(model, data_directory, epoch, img_size, batch_size, learning_rate, patience, validation_split):
    ...
    model.compile(optimizer=Adamoptimizer, loss='categorical_crossentropy', metrics=['accuracy' , 'Recall' , 'TruePositives'])
...
def generate_dataset_from_dataframe( ):
    ...
    train_gen = datagen.flow_from_dataframe(df, directory= directory,  x_col='image_name', y_col='label' , class_mode='raw' , subset='training')
    val_gen = datagen.flow_from_dataframe(df, directory= directory,  x_col='image_name', y_col='label' , class_mode='raw' , subset='validation')
...
def train_test_dataframe_model(model, epoch, learning_rate):
    ...
    model.compile(optimizer=Adamoptimizer, loss='binary_crossentropy', metrics=['accuracy' , 'Recall' , 'TruePositives'])
